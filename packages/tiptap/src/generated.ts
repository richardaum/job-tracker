...
export function normalizeGeneratedTipTapDocument(note: TipTapDocument, fallbackText: string): TipTapDocument {
  const noteRecord = asRecord(note);
  const contentRaw = Array.isArray(noteRecord?.content) ? noteRecord.content : [];
  const paragraphs: Array<{ type: "paragraph"; content: Array<{ type: "text"; text: string }> }> = [];
  ...
  if (paragraphs.length === 0) {
    ...
    return (asRecord(fallback) ?? { type: "doc", content: [] }) as TipTapDocument;
  }
