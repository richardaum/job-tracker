...
/** Extension port and URLs — written to `apps/extension/.env.development`. */
...
export function writeWorktreeAppEnvs(params: {
  ...
  const apiBase = existsSync(mainApiEnvPath)
    ? parseEnvFile(readFileSync(mainApiEnvPath, "utf8"))
    : {};
  const apiOverrides = buildWorktreeApiEnv({
    ports,
    databaseUrl,
    e2eDatabaseUrl,
  });
  ...
  writeFileSync(
    ...
    formatEnvFile(extensionEnv, [
      "# Worktree env — generated by pnpm worktree:setup.",
      "# Overwrites default .env.development.",
    ]),
    "utf8",
  );

  return {
    apiEnvPath: apiBasePath,
    webEnvPath,
    storybookEnvPath,
    extensionEnvPath,
  };
