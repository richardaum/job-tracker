...
- **Implementation baseline:** Extension and related integration (**`apps/extension`**, web **`/sources`**, GraphQL-over-SSE wiring) are treated as **starting from scratch**. Do not assume prior partial implementations, abandoned tooling, or old milestones satisfy **[T-137]**–**[T-138]** until this spec’s acceptance criteria pass on the current codebase.
...
- **Extension UX:** **Popup** + side-panel **mapping wizard** (**D-2**, **D-6**); no third-party DOM injection (**[P-116]**); **latest source round** in popup.
...
- **Web:** **`/sources`** — **single page** (**D-4**): **three-level UX** — (1) **templates grouped by importer** (section titles + tap a template to open **level 2**); (2) **importer-scope detail** in a **side panel** (**See runs**, **Actions**); (3) **modal** listing runs with per-run actions (applications link, unlink, delete). **New template** opens an importer picker (each run stays tied to its template; first run after create opens that importer in the panel). **Live UI** via GraphQL **subscriptions** (**D-5**, **[T-136]**) when wired (separate surface from the extension SSE channel unless later unified).
...
- [P-109] Deliver extension-assisted capture from supported boards into **`New`** applications with source summaries in both extension and web (not draft-and-confirm).
...
- Guest extension onboarding; **[P-81]** on this path; **`specs/016-*`** defining extension behaviour (**[P-120]**); cross-user importer sharing in v1 (**[P-122]**); parallel REST (**[P-121]**); auth paths **[P-119] B/C**; **`/sources`** live UI **without** subscriptions (**[T-136]**); HMAC-signed HTTP body beyond parity.
...
- [P-53] **Allowlist (**D-1**):** **LinkedIn**, **Jack**, **RemoteYeah** — bind **`host_permissions` / URL path matchers** during implementation. **Learning:** importer inference plus **side-panel wizard** (**D-6**) without injecting UI on board pages. **Source modality** (choose extractors + UX for a run) is **determined from `Source` + active tab URL** (and importer config):
...
- [P-109] Create applications as **`New`** — **no source draft** step.
- [P-115] **Extension:** surface **latest source round** until the next begins (**[P-53]** Type L rounds may aggregate **many** job creates; Type J typically **one**). Persist **runs/rounds** server-side (`run` SHOULD record modality + counts). **Concurrent runs:** Multiple rounds—any mix of **Type L** / **Type J**—MAY run **in parallel**; an importer/source MAY **restrict** parallelism (single-flight, throttling—document next to importer rules); where no such restriction applies, **overlapping** rounds is **the user’s choice**. **Web `/sources` (**D-4**): **no tabs** — **grouped list by importer** (left/main column) opens an **importer detail pane from the side** (desktop) listing **templates**, **See runs**, and **Actions**; **runs** stay in a **modal** (level 3); primary CTA **New template** (importer picker); **schedule** (**cron + enabled flag**) and **run again** from the importer template list (stored-only schedule until execution exists server-side). **Live** updates via thin subscription (**[T-136]** / **D-5**) when backend is connected. Expose **`idempotencyKey`** on source-related creates (**D-3**)—scope **one key per persisted application seed\*\* when batching Type L unless API defines a batch key contract.
