...
T-182  Match analysis page (generate button, empty state, results)    —

T-176  GraphQL resume CRUD                                → T-175
T-186  Verify preferences column                          → T-175 (merged into T-176)
T-183  Mutation.generateApplicationMatch + Query.appMatch     → T-192, T-175

T-180  Wire web hooks (resume)                            → T-176, T-177, T-178, T-179
T-184  Wire web hooks (match)                             → T-183, T-182
...
| 1c    | **T-192** — MatchAnalysis entity + migration            | —            | 1d   |
| 1d    | **T-183** — generateApplicationMatch + applicationMatch | T-192, T-175 | 2.5d |
...
| Batch | Tasks                                                      | Deps         | Est. |
| ----- | ---------------------------------------------------------- | ------------ | ---- |
| 3a    | **T-182** — Match analysis page (`/applications/[id]/fit`) | —            | 1.5d |
...
| **C** | T-180 + T-184                                                 | Wire hooks; separate domains (resume vs match)    |
| **D** | T-181 + T-185 + T-188                                         | Independent test suites                           |
| **E** | T-189 + T-191                                                 | Independent UI atop match modal                   |

## Critical path

```
T-175 → T-176 → T-180 → T-181       (Resume CRUD)
T-192 → T-183 → T-184 → T-185       (Match analysis)
```
...
| **Match analysis como página, não modal**         | Mais espaço para os cards de match/gap/unclear + filtros. Consistente com notes sub-page pattern. |

## Integration points

1. **T-176** → consumed by T-180 (web needs resume CRUD hooks)
2. **T-183** → consumed by T-184 (web needs match mutation + query)
