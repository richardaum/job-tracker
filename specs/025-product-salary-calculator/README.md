...
- [P-94] Users can convert between hourly, monthly, and yearly salary rates to quickly compare salary offers across different pay periods.
...
- [P-97] Users can enter a rate in any period field and see the equivalent values in the other two periods instantly.
- [P-98] Users can select source and target currencies and see converted amounts for all three periods simultaneously.
...
- [P-104] A user can convert a salary from USD to EUR and see all three periods (hourly/monthly/yearly) in the target currency.
- [P-105] Exchange rates load within 3 seconds on page load with a visible loading state.
- [P-106] If the currency API fails, the page shows a clear error message with a retry button and still allows period-only conversions (hourly/monthly/yearly without FX).
