...
# Technical Scope: ai-job-create-v2

> **Status**: in_progress · **Priority**: medium · **Created**: 2026-05-07

## Spec

- [T-1] Introduce `AiJobCreatePageV2` in `apps/web/src/modules/jobs/create-ai/page/` and use it as the route entrypoint for `/jobs/new/ai` while the legacy page is phased out.
- [T-2] Implement a functional paste-capture list in `AiJobCreatePageV2`: each distinct paste event while the page is focused creates a new list item at the top with icon + preview: primary `lineOne` single-line clamp; secondary `lineTwo` up to **two** visible lines (`line-clamp-2`) (duplicate payloads skipped per [T-5]).
...
- [T-6] When a duplicate paste is detected and skipped, show a non-blocking info toast in `AiJobCreatePageV2` to inform the user that the pasted content already exists in the list.
...
- [T-8] For each captured paste item in `AiJobCreatePageV2`, display a compact source badge that distinguishes clipboard payload kind as `HTML` when `text/html` content is present, otherwise `Plain text`.
...
- UI composition (`AiJobCreatePageV2`).
