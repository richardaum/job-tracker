...
function CompactBadge({
  ...
  const content = (
    ...
    >
      {label}
      {scoreRatio != null && (
        <span className={cn("ml-1 font-normal opacity-80")}>({Math.round(scoreRatio)}%)</span>
      )}
    </Text>
  );
...
function BadgeBadge({
  ...
  const badge = (
    <Badge
      ...
      className={cn("cursor-pointer font-medium transition-all hover:brightness-95")}
    >
      ...
      {scoreRatio !== null && (
        <div className={cn("rounded-lg border border-border-subtle bg-surface px-3 py-1.5 w-fit")}>
          <Text size="sm" color="secondary" className={cn("whitespace-nowrap")}>
            Score:{" "}
            <span className={cn("font-bold text-text-primary")}>{Math.round(scoreRatio)}%</span>
          </Text>
        </div>
      )}
      <div className={cn("rounded-lg border border-border-subtle bg-surface px-3 py-1.5 w-fit")}>
        <Text size="sm" color="secondary" className={cn("whitespace-nowrap")}>
          ...
          {matchCount !== 1 ? "es" : ""} ·{" "}
          <span className={cn("font-medium text-text-primary")}>{gapCount}</span> gap
          {gapCount !== 1 ? "s" : ""} ·{" "}
          <span className={cn("font-medium text-text-primary")}>{unclearCount}</span> unclear
        </Text>
      </div>
    </div>
  );
