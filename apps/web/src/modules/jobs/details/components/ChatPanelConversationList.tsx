...
import { cn, ConfirmDialog, IconButton, Text, TruncateText } from "@job-tracker/ui";
      ...
      {!isNarrow ? (
        <div className={cn("shrink-0 border-b border-border-subtle p-2")}>
          <button
            ...
            className={cn(
              "flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm font-medium text-text-brand transition-colors hover:bg-bg-surface-hover",
            )}
          >
            ...
            {conversations.map((conv) => (
              <div
                ...
                className={cn(
                  ...
                  conv.id === activeConversationId ? "bg-bg-surface-hover font-medium text-text-primary" : "text-text-muted",
                )}
