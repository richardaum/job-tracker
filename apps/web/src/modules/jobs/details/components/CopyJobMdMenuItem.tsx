...
export interface CopyJobMdMenuItemProps {
  jobId: string;
  job: JobQuery["job"];
}
...
export function CopyJobMdMenuItem({ jobId, job }: CopyJobMdMenuItemProps) {
  ...
  async function handleCopy() {
    ...
    try {
      ...
      enqueueToast({ title: "Copied as Markdown", intent: "success" });
    } catch {
      enqueueToast({ title: "Failed to copy job as Markdown", intent: "error" });
    } finally {
      setIsCopying(false);
    }
  }
  ...
  return (
    <DropdownMenuItem
      ...
      icon={<CopyIcon size={14} weight="regular" />}
    >
