...
export function useMatchTabViewModel(jobId: string) {
  ...
  const { matchAnalysis, matchPk, status, matchLoading, matchError, refetchJobMatch } = useJobMatchStatus();
  ...
  async function handleGenerate(resumeId: string) {
    const [mutationError] = await tryRun(
      generateJobMatch({ variables: { input: { jobId, resumeId } }, refetchQueries: ["JobMatch", "Job"] }),
    );
    if (mutationError) {
      const message =
