...
type PlanDetailsLayoutProps = { children: ReactNode; params: Promise<{ planId: string }> };

export default function PlanDetailsLayout({ children, params }: PlanDetailsLayoutProps) {
  ...
  return (
    <SlotsProvider>
      <div className={cn("flex h-full min-h-0 flex-col")}>
        <DetailPageHeader
          trailing={<PlanHeaderActions.Slot className={cn("flex shrink-0 items-center gap-2 empty:hidden")} />}
        >
          <BackToLink href={"/sources/plans" as Route} className={cn("self-start")}>
            Back to plans
          </BackToLink>
          <DetailPageHeader.Title>{plan?.displayName ?? "Plan"}</DetailPageHeader.Title>
        ...
        <div className={cn("flex flex-1 min-h-0 flex-col gap-4 p-4 sm:p-6")}>
          ...
          <div className={cn("flex min-h-0 min-w-0 flex-1 flex-col overflow-auto pe-2 pb-1 text-start")}>
            {loading ? (
