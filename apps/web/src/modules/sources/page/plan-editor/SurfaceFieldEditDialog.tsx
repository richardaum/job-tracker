...
type SurfaceFieldEditDialogProps = {
  ...
  onDelete?: (field: SurfaceField) => void;
};
...
export function SurfaceFieldEditDialog({
  ...
  return (
    <Dialog
      ...
      description={field ? `Editing "${field.key}".` : "Define a new surface field to extract from the listing row."}
    ...
    >
      <div className={cn("flex-1 overflow-auto pe-3")}>
        <div className={cn("flex flex-col gap-4")}>
          <FormField
            label="Key"
            tooltip={<FieldTooltip content="Identifier used to reference this field elsewhere in the plan." />}
            htmlFor="sf-key"
            required
          >
            <Input
            ...
            <Select
              ...
              onValueChange={(v) =>
                setDraft((prev) => ({
                  ...
                  sourceField:
                    v === "regex" && !prev.sourceField ? (availableKeys?.[0] ?? undefined) : prev.sourceField,
                }))
              }
            />
          </FormField>
          ...
          {draft.type !== "regex" ? (
            <FormField
              label="Selector"
              tooltip={<FieldTooltip content="CSS selector for the element containing this field's data." />}
              htmlFor="sf-selector"
              required
            >
              <Textarea
            ...
            <FormField
              label="Source Field"
              tooltip={<FieldTooltip content="Which field to parse with this regex. Leave empty to use all text." />}
              htmlFor="sf-source"
            >
              <Select
