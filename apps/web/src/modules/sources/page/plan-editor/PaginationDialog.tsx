...
export function PaginationDialog({ step, open, onOpenChange, onSave }: PaginationDialogProps) {
  ...
  const [draft, setDraft] = useState<Pagination>(existing ?? { containerSelector: "", nextButtonPartialMatch: "Next" });

  function mergeIntoStep() {
    const action = step.action as { kind: "collect.jobs"; input: CollectJobsInput };
    return {
      ...step,
      action: { kind: "collect.jobs" as const, input: { ...action.input, pagination: enabled ? draft : undefined } },
    };
  }
          ...
          {enabled && (
            <>
              <FormField
                ...
                tooltip={<FieldTooltip content="CSS selector for the pagination navigation element." />}
              ...
              <FormField
                ...
                tooltip={<FieldTooltip content="Partial text match for the next-page button, e.g. 'Next'." />}
              ...
              >
                <Input
                  ...
                  onChange={(e) => setDraft((prev) => ({ ...prev, nextButtonPartialMatch: e.target.value }))}
