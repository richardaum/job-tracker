  ...
  return (
    <SlotsProvider>
      <div className={cn("flex h-full min-h-0 flex-col")}>
        <DetailPageHeader
          trailing={<SourcesHeaderActions.Slot className={cn("flex shrink-0 items-center gap-2 empty:hidden")} />}
        >
        ...
        <div className={cn("flex min-h-0 flex-1 flex-col")}>
          <div className={cn("flex min-h-0 min-w-0 flex-1 flex-col items-stretch overflow-auto")}>{children}</div>
        </div>
      </div>
    </SlotsProvider>
  );
