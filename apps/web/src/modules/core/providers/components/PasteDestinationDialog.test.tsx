...
vi.mock("@/gql/hooks", () => ({ useSettingsQuery: (...args: unknown[]) => useSettingsQueryMock(...args) }));
...
describe("PasteDestinationDialog", () => {
  ...
  it("defaults after-create checkboxes to settings", () => {
    useSettingsQueryMock.mockReturnValue({
      data: {
        settings: { autoFillEnabled: true, autoSummaryEnabled: false, autoMatchEnabled: true, duplicateWindowDays: 30 },
      },
    });
  ...
  it("defaults after-create checkboxes to unchecked when settings are off", () => {
    ...
    expect(screen.getByRole("checkbox", { name: "Fill job fields automatically" })).not.toBeChecked();
  ...
  it("passes toggled after-create values to onConfirm", async () => {
    useSettingsQueryMock.mockReturnValue({
      data: {
        settings: { autoFillEnabled: true, autoMatchEnabled: true, autoSummaryEnabled: false, duplicateWindowDays: 30 },
      },
    });
    ...
    expect(onConfirm).toHaveBeenCalledWith("", { autoFill: false, autoMatch: false });
  });
});
