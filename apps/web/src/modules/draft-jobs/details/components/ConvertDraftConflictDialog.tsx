...
export function ConvertDraftConflictDialog({
  ...
  const [deleteJobsForDraft] = useDeleteJobsForDraftMutation({
    refetchQueries: [{ query: JobsDocument }],
  ...
  async function createFromDraft() {
    ...
    const [createError] = await tryRun(
      createJobWithAI({ variables: { draftId } }),
    );
  ...
  async function handleReplaceAll() {
    ...
    const [purgeError] = await tryRun(
      deleteJobsForDraft({ variables: { draftId } }),
    );

    if (purgeError) {
      onError?.(purgeError.message || "Failed to delete jobs for this draft.");
      setAction(null);
      return;
    }
