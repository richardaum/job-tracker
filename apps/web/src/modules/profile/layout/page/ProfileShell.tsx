  ...
  return (
    <PortalSlotsProvider>
      <div className={cn("flex h-full min-h-0 flex-col")}>
        <DetailPageHeader
          trailing={<ProfileHeaderActions.Slot className={cn("flex shrink-0 items-center gap-2 empty:hidden")} />}
        >
        ...
        <div className={cn("flex flex-1 min-h-0 flex-col gap-4 p-4 sm:p-6")}>
          <div className={cn("flex flex-wrap items-center gap-x-4 gap-y-2")}>
            <Tabs value={currentTab} onValueChange={navigateToTab} className={cn("w-full sm:w-fit")}>
          ...
          <div
            className={cn("flex min-h-0 min-w-0 flex-1 flex-col items-stretch overflow-auto pe-4 sm:pe-6 text-start")}
          >
            {children}
          </div>
        </div>
