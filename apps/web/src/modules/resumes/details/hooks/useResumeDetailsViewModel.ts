...
import { ResumeDocument, ResumesDocument, useResumeQuery, useUpdateResumeMutation } from "@/gql/hooks";
...
export function useResumeDetailsViewModel(resumeId: string, onSaved: () => void) {
  const { data, loading, error } = useResumeQuery({ variables: { id: resumeId }, fetchPolicy: "cache-and-network" });
  const [updateResume] = useUpdateResumeMutation({
    refetchQueries: [{ query: ResumesDocument }, { query: ResumeDocument, variables: { id: resumeId } }],
  ...
  const [draftState, setDraftState] = useState<{ resumeId: string | null; title: string; content: string }>({
    resumeId: null,
    title: "",
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await updateResume({ variables: { id: resume.id, input: { title: titleDraft, content: contentDraft } } });
  ...
  async function persistResumeTitle(trimmedTitle: string) {
    if (!resume) return;
    await updateResume({ variables: { id: resume.id, input: { title: trimmedTitle } } });
    commitDraft(trimmedTitle, contentDraft);
  }

  async function setResumeDefault() {
    if (!resume) return;
    await updateResume({ variables: { id: resume.id, input: { isDefault: true } } });
  }
