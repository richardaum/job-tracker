...
interface FitWizardDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onGenerate: (resumeId: string) => Promise<void>;
  generating: boolean;
  hasExistingFit: boolean;
}
...
export function FitWizardDialog({
  open,
  onOpenChange,
  onGenerate,
  generating,
  hasExistingFit,
}: FitWizardDialogProps) {
      ...
      footer={
        <div className={cn("flex items-center justify-between gap-2")}>
          ...
          <div className={cn("flex items-center gap-2")}>
            ...
            >
              {hasExistingFit ? "Regenerate" : "Generate"}
            </Button>
          </div>
        </div>
      }
        ...
        <TabsContent value="preferences" className={cn("flex flex-col gap-2")}>
          <div className={cn("mb-2")}>
            <Text size="sm" color="muted">
              ...
              evaluate fit against job descriptions.
            </Text>
          </div>
          {prefsLoading ? (
            <Text size="sm" color="muted">
