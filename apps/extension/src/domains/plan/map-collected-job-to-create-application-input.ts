import { isParsedSalaryForCreateApplicationInput } from "@/domains/dom/parse-salary-inner-text-for-application";
import type { Job } from "@/domains/dom/types";
import { ApplicationSource, type CreateApplicationInput } from "@/gql/graphql";
...
/**
 * CreateApplicationInput coverage from scraped `Job` (see `remoteyeah.plan.json` keys):
 *
...
function inferApplicationSourceFromUrls(
  urls: string[],
  ...
  for (const url of urls) {
    ...
    if (lower.includes("linkedin")) return ApplicationSource.Linkedin;
    if (lower.includes("jack")) return ApplicationSource.Jack;
    if (lower.includes("wellfound")) return ApplicationSource.Wellfound;
    if (lower.includes("remoteyeah")) return ApplicationSource.RemoteYeah;
  }
  return undefined;
}

export function mapCollectedJobToCreateApplicationInput(
  job: Job,
  ...
  const salary = isParsedSalaryForCreateApplicationInput(job.salary)
    ? job.salary
    : undefined;
  const source =
    detailUrl != null ? inferApplicationSourceFromUrls([detailUrl]) : undefined;
