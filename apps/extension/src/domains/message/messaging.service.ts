import { EventPayloadSchemas, MessageEnvelopeSchema, RequestPayloadSchemas } from "./schema";
import type { EventPayloadByType, EventType, RequestPayloadByType, RequestType, RuntimeKind } from "./types";

type RequestHandler = (payload: RequestPayloadByType[RequestType]) => Promise<unknown> | unknown;
type EventHandler<K extends EventType> = (payload: EventPayloadByType[K]) => Promise<void> | void;

type RuntimeAdapter = { kind: RuntimeKind };

type RequestOptions<K extends RequestType> = { to: RuntimeKind; payload: RequestPayloadByType[K]; tabId?: number };

type PublishOptions<K extends EventType> = { to: RuntimeKind; kind: K; payload: EventPayloadByType[K]; tabId?: number };
...
export class MessagingService {
  ...
  async request<K extends RequestType, R = unknown>({ to, payload, tabId }: RequestOptions<K>): Promise<R> {
    ...
    if (to === "content") {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          ...
          const isChannelClosed = err instanceof Error && err.message.includes("message channel closed");
  ...
  async publish<K extends EventType>({ to, kind, payload, tabId }: PublishOptions<K>): Promise<void> {
    ...
    chrome.runtime.onMessage.addListener((rawMessage, _sender, sendResponse) => {
      ...
      if (envelope.mode === "event") {
        ...
        for (const handler of handlers) {
          void Promise.resolve(handler(payloadResult.data as EventPayloadByType[typeof eventKind]));
        }
      }
    });
  }
