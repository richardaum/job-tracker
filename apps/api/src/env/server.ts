...
export const apiEnv = apiEnvSchema
  ...
  .refine(
    ...
    {
      message: "DEV_AUTH_BYPASS_EMAIL is required when AUTH_BYPASS_ENABLED is true.",
      path: ["DEV_AUTH_BYPASS_EMAIL"],
    },
  )
  .refine(
    ({ JWT_ACCESS_SECRET, JWT_ACCESS_SECRETS }) =>
      JWT_ACCESS_SECRETS !== undefined || JWT_ACCESS_SECRET !== undefined,
    {
      message: "JWT_ACCESS_SECRET or JWT_ACCESS_SECRETS is required.",
  ...
  .refine(
    ({ NODE_ENV, RATE_LIMIT_DISABLED }) => NODE_ENV !== "production" || !RATE_LIMIT_DISABLED,
    {
...
export const jwtAccessSecrets = parseJwtSecretPair(
  apiEnv.JWT_ACCESS_SECRETS,
  apiEnv.JWT_ACCESS_SECRET,
);

export const jwtRefreshSecrets = parseJwtSecretPair(
  apiEnv.JWT_REFRESH_SECRETS,
  apiEnv.JWT_REFRESH_SECRET,
);
