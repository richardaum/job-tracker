...
const hasDbUrl = () =>
  !!(process.env.DATABASE_INTEGRATION_URL || process.env.DATABASE_URL);
...
describe.runIf(hasDbUrl())("AddKeywordBlockerColumns migration", () => {
  ...
  async function cleanColumns(dataSource: import("typeorm").DataSource) {
    await dataSource.query(
      `ALTER TABLE "user_settings" DROP COLUMN IF EXISTS "blocked_companies"`,
    );
    await dataSource.query(
      `ALTER TABLE "user_settings" DROP COLUMN IF EXISTS "blocked_keywords"`,
    );
  }

  async function cleanMigrationRecord(
  ...
  it("migration up and down work correctly", async () => {
    ...
    try {
      ...
      expect(
        columnsAfterRollback?.findColumnByName("blocked_keywords"),
      ).toBeUndefined();
      expect(
        columnsAfterRollback?.findColumnByName("blocked_companies"),
      ).toBeUndefined();
    ...
    try {
      ...
      await dataSource.query(
        ...
        [
          "test-user-176800",
          JSON.stringify([
            { keyword: "test", scope: "TITLE", matchMode: "PARTIAL" },
          ]),
      ...
      expect(row[0].blocked_keywords).toEqual([
        { keyword: "test", scope: "TITLE", matchMode: "PARTIAL" },
      ]);
      expect(row[0].blocked_companies).toEqual(["Acme Corp"]);

      await dataSource.query(
        `DELETE FROM "user_settings" WHERE "user_id" = 'test-user-176800'`,
      );
