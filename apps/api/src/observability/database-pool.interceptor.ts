...
export class DatabasePoolInterceptor {
  ...
  private patchConnect(pool: Pool): void {
    const originalConnect = pool.connect.bind(pool) as { (callback: ConnectCallback): void; (): Promise<PoolClient> };
  ...
  private logSlowIfNeeded(startedAt: number, queryText: string | undefined) {
    ...
    this.logger.warn(`[db][slow-query] ${elapsedMs}ms${normalizedQuery ? ` query="${normalizedQuery}"` : ""}`);
  }
