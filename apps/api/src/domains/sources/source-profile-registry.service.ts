...
const SOURCE_PROFILE_DISPLAY_NAME: Readonly<Record<string, string>> =
  Object.freeze({ remoteyeah: "RemoteYeah" });

const PLAN_BY_SOURCE_PROFILE_ID: Readonly<
  Record<string, ExecutorPlanDocument>
...
export class SourceProfileRegistryService {
  ...
  listSourceProfileDescriptors(): SourceProfileDescriptorRow[] {
    const rows = Object.keys(PLAN_BY_SOURCE_PROFILE_ID).map(
      (sourceProfileId) => ({
        sourceProfileId,
        name:
