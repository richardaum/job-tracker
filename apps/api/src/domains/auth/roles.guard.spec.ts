...
describe("RolesGuard", () => {
  it("allows when DB role matches required", async () => {
    const reflector = { getAllAndOverride: vi.fn().mockReturnValue([RoleEnum.User]) } as unknown as Reflector;
    const userService = { findById: vi.fn().mockResolvedValue({ role: RoleEnum.User }) } as unknown as UserService;
  ...
  it("blocks when DB role does not match required", async () => {
    const reflector = { getAllAndOverride: vi.fn().mockReturnValue([RoleEnum.Admin]) } as unknown as Reflector;
    const userService = { findById: vi.fn().mockResolvedValue({ role: RoleEnum.User }) } as unknown as UserService;
    const guard = makeGuard(reflector, userService);
  ...
  it("allows admin regardless of requiredRoles", async () => {
    const reflector = { getAllAndOverride: vi.fn().mockReturnValue([RoleEnum.User]) } as unknown as Reflector;
    const userService = { findById: vi.fn().mockResolvedValue({ role: RoleEnum.Admin }) } as unknown as UserService;
    const guard = makeGuard(reflector, userService);
