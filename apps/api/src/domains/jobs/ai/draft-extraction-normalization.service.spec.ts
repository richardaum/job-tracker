...
describe("DraftExtractionNormalizationService", () => {
  ...
  describe("normalizeExtraction", () => {
    ...
    it("handles missing salary object → salary with nulls", () => {
      const result = service.normalizeExtraction({ title: "Engineer", company: "Acme" });
    ...
    it("fills title and company defaults when omitted", () => {
      const result = service.normalizeExtraction({ description: "Some text", tags: [] });
    ...
    it("returns null description when description field is non-string JSON value", () => {
      expect(
        service.normalizeExtraction({ title: "T", description: 42 } as Record<string, unknown>).description,
      ).toBeNull();
    });

    it("normalizes description that is already TipTap JSON", () => {
      const doc = { type: "doc", content: [{ type: "paragraph", content: [{ type: "text", text: "Hello " }] }] };
      const result = service.normalizeExtraction({ title: "T", description: JSON.stringify(doc) });
    ...
    it("handles unicode and HTML-like plain text descriptions", () => {
      ...
      const result = service.normalizeExtraction({ title: "T", description: text });
