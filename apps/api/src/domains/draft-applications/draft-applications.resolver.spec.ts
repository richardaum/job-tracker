...
describe("DraftApplicationsResolver (integration)", () => {
  ...
  beforeAll(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      ...
      providers: [
        ...
        {
          provide: DraftApplicationsService,
          useValue: {
            ...service,
            create: vi.fn(),
            delete: vi.fn(),
            deleteAllLinkedApplications: vi.fn(),
            update: vi.fn(),
          },
        },
      ],
  ...
  it("draftApplication maps NotFound to NOT_FOUND", async () => {
    service.findOne.mockRejectedValueOnce(
      new NotFoundException("Draft application x not found"),
    );
    const res = await request(app.getHttpServer())
      .post("/graphql")
      .set(auth)
      .send({ query: `{ draftApplication(id: "x") { id } }` });
  ...
  it("draftApplication masks ForbiddenException as NOT_FOUND", async () => {
    service.findOne.mockRejectedValueOnce(new ForbiddenException());
    const res = await request(app.getHttpServer())
      .post("/graphql")
      .set(auth)
      .send({ query: `{ draftApplication(id: "d") { id } }` });
