  ...
  () => {
    ...
    afterAll(async () => {
      if (dataSource?.isInitialized) {
        await dataSource.query(
          "TRUNCATE match_analysis, jobs, resumes, companies, users CASCADE",
        );
    ...
    describe("generation_status column via raw SQL", () => {
      it("reads generation_status via snake_case column mapping", async () => {
        ...
        expect(row[0].generation_status).toBe(
    ...
    describe("updateById with expectedStatus", () => {
      it("updates when expectedStatus matches", async () => {
        ...
        expect(updated?.generationMetadata?.status).toBe(
    ...
    describe("resetStaleProcessing", () => {
      it("resets PROCESSING → FAILED, ignores others", async () => {
        ...
        expect(byId[m1.id].generation_status).toBe(
          AsyncMetadataStatusEnum.FAILED,
        );
        expect(byId[m1.id].generation_error).toBeTruthy();
        expect(byId[m2.id].generation_status).toBe(
          AsyncMetadataStatusEnum.FAILED,
        );
        expect(byId[m3.id].generation_status).toBe(
          AsyncMetadataStatusEnum.COMPLETED,
        );
      });
    });
