...
describe.skipIf(!hasDb)("MatchAnalysisRepository — generation metadata (integration)", () => {
  ...
  beforeAll(async () => {
    ...
    const resume = await resumeRepo.save(resumeRepo.create({ userId, title: "Test Resume", isDefault: true }));
    resumeId = resume.id;
  });
  ...
  async function createJob() {
    ...
    const company = await companyRepo.save(companyRepo.create({ userId, name: `Match Corp ${jobCounter}` }));
    const jobRepo = dataSource.getRepository(JobEntity);
    return jobRepo.save(
      jobRepo.create({ userId, title: `Match Test Job ${jobCounter}`, companyId: company.id, urls: [] }),
    );
  }
  ...
  describe("generation_status column via raw SQL", () => {
    ...
    it("generation_status IS NULL for never-requested analysis", async () => {
      const job = await createJob();
      const row = dataSource.getRepository(MatchAnalysisEntity).create({ userId, resumeId, jobId: job.id, items: [] });
      const saved = await dataSource.getRepository(MatchAnalysisEntity).save(row);

      const result = await dataSource.query(`SELECT generation_status FROM match_analysis WHERE id = $1`, [saved.id]);
  ...
  describe("resetStaleProcessing", () => {
    it("resets PROCESSING → FAILED, ignores others", async () => {
      ...
      const rows: Array<{ id: string; generation_status: string; generation_error: string | null }> =
