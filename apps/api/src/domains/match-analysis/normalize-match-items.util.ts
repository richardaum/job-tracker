...
export function normalizeMatchSource(source: string | null | undefined): MatchSourceEnum | undefined {
...
export function normalizeMatchVerdict(verdict: string | null | undefined): MatchVerdictEnum | undefined {
...
export function normalizeRequirementType(type: string | null | undefined): RequirementTypeEnum | undefined {
  if (!type) return undefined;
  const normalizedKey = type.toLowerCase().replace(/_/g, "");
  return Object.values(RequirementTypeEnum).find((value) => value.toLowerCase().replace(/_/g, "") === normalizedKey);
}
...
export function normalizeMatchItem(item: MatchItem): MatchItem {
  return {
    ...
    verdict: item.verdict ? (normalizeMatchVerdict(String(item.verdict)) ?? item.verdict) : item.verdict,
  };
}

export function normalizeMatchAnalysisEntity(entity: MatchAnalysisEntity | null): MatchAnalysisEntity | null {
