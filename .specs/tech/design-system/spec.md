...
### Condensed Density Policy (Product-level)

The web app is intentionally dense. Components must prioritize information density while preserving readability and tap/focus safety.

| Context                              | Default density choice                                    | When to use expanded                                 |
| ------------------------------------ | --------------------------------------------------------- | ---------------------------------------------------- |
| Vertical lists (cards, rows)         | `Stack gap="sm"` or tighter (`gap="xs"`)                  | Only for marketing-like sections                     |
| Loading placeholders (skeleton list) | `Card padding: sm` + internal `Stack gap="xs"`/`gap="sm"` | Never use max card spacing by default                |
| Standard content cards               | `Card padding: md`                                        | Use `sm` when list density is the priority           |
| Section-to-section spacing           | `space-section-gap`                                       | Do not exceed unless required by responsive behavior |

**Rules:**

- Condensed wins by default in authenticated/productive flows (`/applications`, tables, forms, lists).
- Achieve density through typography and semantic spacing tokens, never by shrinking text below defined semantic roles.
- For repeated list surfaces, prefer reducing `card gap` and `card padding` before reducing font sizes.
- If a screen feels sparse, adjust spacing tokens/components first; avoid introducing custom one-off spacing values.
- `Stack` uses generic density scale for layout spacing: `xs` (8px), `sm` (12px), `md` (16px), `lg` (24px).
...
| DS-11 | Authenticated product flows are condensed-by-default: semantic spacing and component padding favor density without reducing legibility |
...
| `Stack`     | Flexbox: `direction`, `gap` (`xs`,`sm`,`md`,`lg`), `align`, `justify` | No              |
