...
### Phase 4: Codegen + UI Components (Parallel)
...
### T07: GraphQL Code Generator extension [P]

**What:** Extend existing GraphQL Code Generator setup in `apps/web` with application CRUD operations
**Where:** `apps/web/codegen.ts`, `apps/web/src/gql/`
**Depends on:** google-oauth T11, T05
**Requirement:** AC-02

**Done when:**

- [ ] Existing `codegen.ts` baseline is reused (do not duplicate setup)
- [ ] `pnpm --filter @job-tracker/web codegen` regenerates typed artifacts in `src/gql/`
- [ ] Generated files include typed `useApplicationsQuery`, `useCreateApplicationMutation`, etc.
- [ ] Existing `codegen` script in `apps/web/package.json` is reused
...
| T07: GraphQL codegen extension | 1 config + generated files         | ✅     |
...
| T05  | T04                     | T04 → T05     | ✅     |
| T06  | T05                     | T05 → T06     | ✅     |
| T07  | google-oauth T11, T05   | T05 → T07     | ✅     |
| T08  | T07                     | T07 → T08     | ✅     |
| T09  | T07                     | T07 → T09     | ✅     |
| T10  | T07                     | T07 → T10     | ✅     |
| T11  | T07                     | T07 → T11     | ✅     |
