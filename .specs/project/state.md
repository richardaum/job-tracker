...
**Current Work:** M2 - Rich Tracking planning. M1 foundation features completed: design-system, visual-identity, and application-crud.
...
**Impact:** `pnpm lint` and `pnpm format:check` are the project gates for lint/format; see `.specs/tech/tooling/lint-format-precommit/` and `.specs/tech/codebase/testing.md` gate table.
...
**Impact:** `DATABASE_URL` is enforced as a PostgreSQL URL on the API; Drizzle config reuses the same export. Web layout loads an extendable empty schema today; new secrets get the same validation and export pattern. See `.specs/tech/config/env-validation-zod-server-only/`.
...
### LL-003: TLC lightweight mode for cross-cutting tooling (2026-04-19)
...
**Fix:** Use a lightweight TLC path for this class of work under `features/` or `tech/` (for example a `task.md` + `summary.md` pair), update `state.md` (decision + task history table), and extend `.specs/tech/codebase/testing.md` gate commands when new verification commands exist.

**Takeaway:** Even small or multi-file chores get **Specify (lightweight) → Execute → Verify → track** per TLC; load `state.md` at session start when the project uses SDD.
...
**Fix:** Added mandatory TLC completion protocol to `.specs/tech/codebase/conventions.md` and evidence checklist to `.specs/tech/codebase/testing.md`; reopened T11-T15 and reset feature status to in progress.
...
### LL-011: `apps/web/next-env.d.ts` may legitimately alternate between `.next` and `.next-dev` (2026-04-21)

**Situation:** Running Playwright/dev flows changed `apps/web/next-env.d.ts` from `./.next/types/routes.d.ts` to `./.next-dev/types/routes.d.ts`, which initially looked like an incidental/generated diff to revert.

**Cause:** `apps/web/next.config.ts` sets `distDir` dynamically: development uses `.next-dev`, non-development uses `.next`. `next-env.d.ts` references whichever routes type path was generated by the last Next run mode.

**Fix:** Treat both references as valid for this project and avoid “cleanup” reverts that only switch between `.next` and `.next-dev` unless there is an explicit repository convention to pin one mode.

**Takeaway:** In this repo, `next-env.d.ts` path flips are expected across dev/build workflows; do not classify this change as accidental by default.
...
| --- | ---------------------------------------------------- | ---------- | -------------------- | ------------------------------------------------------------------------------ |
| 001 | ESLint + Prettier + Husky + lint-staged (monorepo)   | 2026-04-19 | `f480363`            | Done — [task.md](../tech/tooling/lint-format-precommit/task.md)                |
| 002 | Zod + server-only server env (API + Web)             | 2026-04-19 | `5be7589`, `e1a73e4` | Done — [task.md](../tech/config/env-validation-zod-server-only/task.md)        |
| 003 | OrbStack install + Dockerfile fixes (T10 gate)       | 2026-04-19 | `c26f9c5`            | Done — [task.md](../tech/infrastructure/orbstack-docker-setup/task.md)         |
| 004 | Remove `server-only` from NestJS API env module      | 2026-04-19 | `7c9ae2c`            | Done — [task.md](../tech/config/remove-server-only-api-env/task.md)            |
| 005 | Load dotenv before Zod env validation in API         | 2026-04-19 | `8f6bb51`            | Done — [task.md](../tech/config/dotenv-api-env-load/task.md)                   |
| 006 | Fix Sentry Next.js SDK warnings                      | 2026-04-19 | `d9ea280`            | Done — [task.md](../tech/observability/sentry-nextjs-warnings/task.md)         |
| 007 | Configurable PORT in 31xx range (web=3100, api=3101) | 2026-04-19 | `f948d93`            | Done — [task.md](../tech/developer-experience/api-port-finder/task.md)         |
| 008 | Web typed env lint + generated codegen hooks         | 2026-04-20 | _pending_            | Done — [task.md](../tech/tooling/web-typed-env-lint-and-codegen-hooks/task.md) |
